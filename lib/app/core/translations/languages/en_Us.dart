const Map<String, String> enUS = {
  'userLogin': 'User Login',
  'userSignUp': 'User Signup',
  'pleaseFillUpTheInformationBelow': 'Please Fill Up The Information Below',
  'login': 'Login',
  'enterYourContactNumber': 'Enter Your Contact Number(+8529000000000)',
  'doNotHaveAccountSignUpNow': 'Don\'t have an account? Signup Now',
  'alreadyHaveAccountSignInNow': 'Already have an account? Login',
  'byContinuingYouAgreeTo': 'By Continuing, You agree to our\n',
  'termsOfService': 'Terms Of Service',
  'and': 'and',
  'privacyPolicy': 'privacy policy',
  'enterYourEmail': 'Enter Your Email',
  "otpSentDescription": "Please verify OTP sent to your phone number",
  'password': 'Password',
  'pleaseEnterEmailAddress': 'Please enter email address',
  'pleaseEnterValidEmailAddress': 'Please enter valid email address',
  'pleaseEnterPassword': 'Please enter password',
  'pleaseEnterValidPassword':
      'Password must consist of a special character, number and minimum 8 char',
  'enterName': 'Enter You Name',
  'enterEmail': 'Enter email address',
  'pleaseEnterPhoneNumber': 'Please enter phone number',
  'pleaseEnterValidNumber': 'Please enter a valid phone number',
  'verificationOTP': 'Verification OTP',
  'aboutUs': 'About Us',
  'aboutUsDescription':
      'AN Bus is a local red minibus company aim at providing new type of red minibus service in Hong Kong. The mobile application is developed by Bytedge. Bytedge provides Intelligent and smart Transportation solutions including ticketing for fleets.',
  'forgotPassword': 'Forgot password?',
  'newPassword': 'New Password',
  'submitPassword': 'Submit Password',
  'getOtp': 'Get OTP',
  'enterOtp': 'Enter OTP',
  'pleaseEnterOtp': 'Please enter OTP',
  'passwordChangedSuccessfully': 'Password Changed Successfully!',
  'selectPickUp': 'Select Your Pickup Stop',
  'selectDestination': 'Select Your Destination Stop',
  'keyPressToExitApp': "Press one more time to push app into background",
  'keyNoResultFound': "No Result Found",
  'keyEnglish': "English",
  'keySimplifiedChinese': "Simplified Chinese",
  'keyTraditionalChinese': "Traditional Chinese",
  'keySelectLanguage': "Select Language",
  'keyChangeLanguage': "Change Language",
  'keyPreferredLang': "Preferred Language",
  'keyBooking': "Booking",
  'keyDropAFeedback': "Drop A Feedback",
  'keyRateUs': "Rate Us",
  'keyLoginSignUp': "Please Login/Signup",
  'keySelectYourDestiny': "Select Your Destiny",
  'keySelectYourArea': "Select Your Area",
  'keySelectYourStop': "Select Your Stop",
  'keyCurrent': "Current",
  'keyCancelled': "Cancelled",
  'keyPast': "Past",
  'keyDeleteAll': 'Delete All',
  'keySelectATripThatSuitsPreference': 'Select a trip that suits your preference',
  'keyNoBusSelectedTryLater': 'No Bus available right now. Please try again later!',
  'keyTotalBookedSeats': 'Total Booked Seats:',
  'keyBookingNo': 'Booking No:',
  'keySorryBusFull': 'Sorry, The Bus Is Full',
  'keyBookNow': 'Book Now',
  'keyBookingAmount': 'Booking Amount',
  'keySelectedSeats': 'Selected Seats',
  'keySelectNumberOfSeat': 'Select Number Of Seat',
  'keyReserveSeats': 'Reserve seats that suits your preferences.',
  'keySelectNormalSeatsPrompt': 'Please select number of normal seats before proceeding',
  'keyRideConfirmed': 'Ride Confirmed',
  'keyYourBusArrive': 'Your Bus Will arrive at',
  'keyReach3MinBefore': '. Please reach the stop 3 minutes before the schedule arrival time.',
  'keyGotIt': 'GOT IT',
  'keyAboutYourExperienceInRides': 'Tell us about your experience during our rides.',
  'keyTypeOfFeedback': 'Type of feedback',
  'keyCommendation': 'Commendation',
  'keyComplaint': 'Complaint',
  'keySuggestion': 'Suggestion',
  'keySelect': 'Select',
  'keyMr': 'Mr',
  'keyMs': 'Ms',
  'keyMrs': 'Mrs',
  'keySelectRoute': 'Select Route',
  'keySelectDate': 'Select Date',
  'keyIncidentDetails': 'Incident Details',
  'keySubmit': 'Submit',
  'keySelectAnOption': 'Select An Option',
  'keyClose': 'Close',
  'keySearchBus': 'Search for suitable bus',
  'keyBookmark': 'Bookmark',
  'keyMyProfile': 'My Profile',
  'keyEditProfileDetails': 'EDIT PROFILE DETAILS',
  'keyLogOut': 'LOGOUT',
  'keyEnterValidEmail': 'Enter a valid email',
  'keyProfileUpdated': 'Profile updated successfully',
  'keyProfilePic': 'Profile picture',
  'keyCamera': 'Camera',
  'keyGallery': 'Gallery',
  'keyEditProfile': 'Edit Profile',
  'keyName': 'Name',
  'keyEmail': 'Email',
  'keyPhone': 'Phone Number',
  'keySave': 'Save Changes',
  'keyLogoutSuccess': 'Logout successfully',
  'keyForgot': 'Forgot Password?',
  'keyWelcome': 'WELCOME ONBOARD TO\nSmartRyde',
  'keySignUp': 'SIGNUP',
  'keySelectCountry': 'Select Country Code',
  'keyTrackBus': 'Track The Bus',
  'keyLiveTracking': 'Live Tracking',
  'keyMap': 'Map',
  'keyStopLocation': 'Stop Location',
  'keyPickAndStop': 'Pickup stop and Destination stop cannot be the same',
  'keyLoggedIn': 'Logged In',
  'keyPickStop': 'Pickup Stop',
  'keyDestinationStop': 'Destination Stop',
  'keyCancelBooking': 'Cancel Booking',
  'keyETA': 'ETA',
  'keyScheduleTime': 'Schedule Time',
  'keyYes': 'Yes',
  'keyBookedSuccess': 'Booked successfully',
  'keyNo': 'No',
  'keySureDelete': 'Are you sure you want to delete all?',
  'keySureSingleDelete': 'Are you sure you want to delete?',
  'keySureRemove': 'Are you sure you want to remove bookmark?',
  'keyNextStep': 'Next Step',
  'keyResend': 'Resend',
  'keyVerify': 'Verify',
  'keyFeedSubmit': 'Feedback submitted successfully',
  'keyOtpSend': 'OTP send successfully',
  'keyInvalidOtp': 'Invalid OTP',
};
